...
Watch the miner eat the CPU. With nothing watching, it just keeps running:

```bash
watch -n 2 kubectl -n shop top pod
```{{exec}}

You'll see CPU climb and stay high — the cluster is now mining for the attacker. Press `Ctrl+C` to stop watching once the spike is obvious.

> This is the "before" picture: an application RCE turned into a running crypto-miner, and nothing detected or stopped it. In the next steps we add detection and response, then attack again.
